...
extern const char *kSmfCaptureScreenKey;
...
#ifndef _WINRT_DLL
//#define SMARTFACE_MUTEX_LOCK(lockmutex) __android_log_print(ANDROID_LOG_DEBUG,"LOCK","LOCK < line=%d file=%s",__LINE__,__FILE__);  pthread_mutex_lock(&lockmutex); __android_log_print(ANDROID_LOG_DEBUG,"LOCK","LOCK > line=%d file=%s",__LINE__,__FILE__)
#define SMARTFACE_MUTEX_LOCK(lockmutex) pthread_mutex_lock(&lockmutex)
#define SMARTFACE_MUTEX_LOCK_(lockmutex) pthread_mutex_lock(&lockmutex)
#else
#define SMARTFACE_MUTEX_LOCK(lockmutex) lockmutex.lock()
#endif

#ifndef _WINRT_DLL
//#define SMARTFACE_MUTEX_UNLOCK(lockmutex) __android_log_print(ANDROID_LOG_DEBUG,"LOCK","UNLOCK < line=%d file=%s",__LINE__,__FILE__); pthread_mutex_unlock(&lockmutex); __android_log_print(ANDROID_LOG_DEBUG,"LOCK","UNLOCK > line=%d file=%s",__LINE__,__FILE__)
#define SMARTFACE_MUTEX_UNLOCK(lockmutex) pthread_mutex_unlock(&lockmutex)
#define SMARTFACE_MUTEX_UNLOCK_(lockmutex) pthread_mutex_unlock(&lockmutex)
#else
#define SMARTFACE_MUTEX_UNLOCK(lockmutex) lockmutex.unlock()
#endif

#ifndef _WINRT_DLL
#define SMARTFACE_COND_BROADCAST(cond) pthread_cond_broadcast(&cond)
#else

#endif

#ifndef _WINRT_DLL
#define SMARTFACE_COND_SIGNAL(cond) pthread_cond_signal(&cond)
#else

#endif

#ifndef _WINRT_DLL
#define SMARTFACE_COND_WAIT(cond, lockmutex) pthread_cond_wait(&cond, &lockmutex)
#else
...
#define JS_INVALIDATE_VIEW(view) view->Invalidate();
    ...
    #define JS_INVALIDATE_VIEW_WITHOUT(view) view->Invalidate(true);
#else
    #define JS_INVALIDATE_VIEW_WITHOUT(view) view->Invalidate();
#endif
