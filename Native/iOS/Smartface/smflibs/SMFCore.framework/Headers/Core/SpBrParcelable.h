...
public:
    ...
    virtual void NotifyControl(SpDataSet *dataset, unsigned int datasetChangeOptions) {};
    virtual void AssignDefaultValue(const char *attribute, const char *value);

    /**
     * this function called when some event fires starting point.
     ...
     * @author adem.cayir
     */
    virtual void FiringEventOnThis(int eventID, int eventCode = -1) { }
    /**
    ...
    virtual bool ObjectRespondsToEvent(const char *eventName);
    virtual void NotifyBindVariable(SpDataBridge* source,SpBrString* value) {};
public:
    int mRefCount;
    bool isDynamicObject;
    pthread_mutex_t refMutex;
    void *externalData;
    SpBrNuiEffects* effects;
protected:
