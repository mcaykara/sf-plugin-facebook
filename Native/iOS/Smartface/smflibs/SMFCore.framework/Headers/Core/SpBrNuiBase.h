...
public:
    SpContext *context;
    SpBrNuiContainer *parentControl;

    SpDataBridge *uiDataBridge;
    SpDataSet *connDataset;
...
public:
    ...
    std::vector<SpGestures*> allGestures;
    bool isOnScreen;
...
public:
    ...
    /*
     * Only call on Java objects that implement AndroidUI.WithBitmap.
     * NULL's are permitted.
     */
    void SetBitmapToAndroid(SpBrString *normalStr,
    		SpBrString *activeStr, SpBrString *pressedStr,
    		SpBrString *disabledStr,bool force=false);
...
public:
    ...
    */
    ...
    void SetDefaultGeometry(int target, float value, GeometryValueTypes valueType);
    /*
    ...
    virtual void SetRect(SpUtRect rect, bool invalidate = true);
    virtual void SetNativeRect(SpUtRect rect);
    SpUtRect GetRect();
    SpUtRect GetNativeRect();
    SpBrNuiPage* GetTopParent();
    int GetInitialWidthConstant();
    int GetInitialHeightConstant();
    SpBrNuiContainer* GetTopAutoSizedParent();
    bool HasAutoSizedParent();
    SpBrNuiContainer* GetTopParentToPerformLayout();
    ...
    void SetFontData(SpJSFontData *fontData);
    void UpdateFontData(SpJSFontData *fontData);
    void MakeFontName(SpBrString* fntName, TFontStyle fntStyle);
    virtual void SetStrikeThrough(bool strikeThru){};
    bool ViewExists();
    bool InLayout();
    bool ParentAutosized();
    virtual void AdjustView() {};
    virtual void AddGesture(SpBrString* gestureId , SpBrString* gestureType, SpBrString* gestureDirection, int touches, int taps);
    virtual void RemoveGesture(SpBrString* gestureId );
#ifdef TARGET_PLATFORM_IOS
    bool AddPluginView(SpJsObject childPluginView);
    void PerformLayoutForViewObjects();
    void SetViewsFrames();
    void AddGesturesAfterLoad();
    int zIndexInSuper;
#endif
...
public:
    ...
    virtual void NotifyFromCustomVariable(SpBrString *value) {};
    void ReadGeometry(SpDataSource* datasource);
    void RemoveGestureFromArray(SpBrString* gestureId);
};
