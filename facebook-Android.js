...
const NativeMessageDigest = requireClass("java.security.MessageDigest");
const NativeBase64 = requireClass("android.util.Base64");
const PackageManager = requireClass("android.content.pm.PackageManager")
...
Object.defineProperties(Facebook.android, {
    ...
    'hashKeys': {
        get: function() {
            var packageManager = spratAndroidActivityInstance.getPackageManager();
            var info = packageManager.getPackageInfo(spratAndroidActivityInstance.getPackageName(), PackageManager.GET_SIGNATURES);
        
            var keys = [];
            var signatures = toJSArray(info.signatures);
            signatures.forEach(function(signature) {
                var md = NativeMessageDigest.getInstance("SHA");
                md.update(signature.toByteArray());
                var key = NativeBase64.encodeToString(md.digest(), NativeBase64.DEFAULT);
                keys.push(key);
            });
            return keys;
        }, 
